...
CREATE INDEX idx_group_members_invitation ON group_members(invitation_expires_at) WHERE status = 'pending';
...
-- Create simplified RLS policies for groups
...
-- Create simplified RLS policies for group members
...
BEGIN
  ...
  SET status = 'expired',
      invitation_metadata = invitation_metadata || jsonb_build_object(
